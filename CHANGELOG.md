...
### Added
- Add `permissionMode` to the `tmuxPane` config defaults, so pane children can be started in Claude's auto mode without repeating it in every profile. It is the one defaultable field that changes what a child may do rather than how it is presented: whether an unattended child stops to ask is a property of how an operator works, not of the agent. `model`, tool allowlists and `extraArgs` stay profile-only.
- Add `reuse` to the `tmuxPane` config defaults, so a new pane profile with nothing in its file can be tried out as a visible, shared, single persistent pane and then promoted one line at a time (`layout: window`, then `interactive: false`). A defaulted `reuse` is withheld from parallel and worktree children instead of refusing them: a default states what the operator usually wants, while `reuse` written in a profile asserts something about that agent and is still refused where it cannot hold. Reuse is now resolved at launch, where that distinction is knowable, and baked into the step.
